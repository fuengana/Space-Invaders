...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        to play a game.
        """
        self.background = GImage(x = GAME_WIDTH / 2, y = GAME_HEIGHT / 2, width
                = GAME_WIDTH, height = GAME_HEIGHT, source = 'A7background.jpg')
        self._wave = None
        self._state = STATE_INACTIVE
        if self._state == STATE_INACTIVE:
            self._text = GLabel(x = GAME_WIDTH / 2, y = GAME_HEIGHT / 2,
                font_size = 40, font_name = "RetroGame.ttf", bold = True,
                linecolor = introcs.RGB(104, 255, 112), text =
                " Space Invaders \nPress 'S' to Start")
        else:
            self._text = None
    ...
    def inactive(self):
        """
        Sets the game to state STATE_NEWWAVE for one frame once 's' is pressed,
        then sets game to state STATE_ACTIVE
        """
        self._wave = None
        if self.input.is_key_down('s') == True:
            self._state = STATE_NEWWAVE
            self._wave = Wave()
            self._state = STATE_ACTIVE


    def active(self, input, dt):
        """
        Sets the state to normal game play. Changes state to STATE_COMPLETE or
        STATE_PAUSED depending on the conditions
        ...
        assert type(dt) == float or int
        self._wave.update(input, dt)
        self._text = None
        if self._wave.game_over() == 0 or self._wave.game_over() == 1:
            self._state = STATE_COMPLETE
        elif self._wave.game_over() == 2:
            self._state = STATE_PAUSED


    def pause(self):
        """
        After the game is paused when a life is lost
        unpauses the game once 's' is pressed
        """
        lives = self._wave.getLives()
        self._text = GLabel(x = GAME_WIDTH / 2, y = GAME_HEIGHT / 2, font_size =
            40, font_name = "RetroGame.ttf", bold = True, linecolor =
            introcs.RGB(104, 255, 112), text =  " Life Lost! \nPress 'S' to Continue")
        if self.input.is_key_down('s') == True:
    ...
    def continue_state(self):
        """
        Restore ship after destroyed and updates state after state is no longer
        paused. State is only in STATE_CONTINUE for one second before it becomes
        STATE_ACTIVE
        """
        self._wave.setShip()
        self._state = STATE_ACTIVE
    ...
    def complete(self):
        """
        Once the wave is over and state is STATE_COMPLETE, determines if player
        has lost the game and the game is over
        """
        if self._wave.game_over() == 0:
            self._text = GLabel(x = GAME_WIDTH / 2, y = GAME_HEIGHT / 2,
            font_size = 40, font_name = "RetroGame.ttf", bold = True,
            linecolor = introcs.RGB(104, 255, 112), text =
            "You Win! \nPress 'S' for next Wave")
            if self.input.is_key_down('s') == True:
                Invaders.n += 1
                lives = self._wave.getLives()
                initial_speed = self._wave.getInitialSpeed()
                self._state = STATE_NEWWAVE
                self._wave = Wave()
                self._wave.next_speed(initial_speed * (0.96 ** Invaders.n))
                self._wave.setSpeed()
                self._wave.lives(lives)
                self._state = STATE_ACTIVE
        elif self._wave.game_over() == 1:
            self._text = GLabel(x = GAME_WIDTH / 2, y = GAME_HEIGHT / 2,
             font_size = 40, font_name = "RetroGame.ttf", bold = True,
            linecolor = introcs.RGB(104, 255, 112), text =
            "GAME OVER... \nPress 'S' to Replay")
