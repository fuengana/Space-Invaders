...
class Ship(GImage):
    ...
    def __init__(self, x, y, width, height, source):
        ...
        Parameter width: The width of the ship image
        Precondition: width is an int or float
        Parameter height: The height of the ship image
        Precondition: height is an int or float

        Parameter source: a string of the file name of the image
        Precondition: source must be a str
        """
        super().__init__(x = x, y = y, width = width, height = height, source =
            source)
    ...
    def collides(self, bolt):
        ...
        w = BOLT_WIDTH / 2
        h = BOLT_HEIGHT / 2
        return bolt.isPlayerBolt() == False and (self.contains((bolt.x - w,
            bolt.y - h)) or self.contains((bolt.x + w, bolt.y - h)) or
            self.contains((bolt.x + w, bolt.y + h)) or self.contains((bolt.x -
            w, bolt.y + h)))
...
class Alien(GImage):
    ...
    def __init__(self, x, y, width, height, source):
        ...
        Parameter width: The width of the alien image
        Precondition: width is an int or float
        Parameter height: The height of the alien image
        Precondition: height is an int or float
    ...
    def collides(self, bolt):
        ...
        w = BOLT_WIDTH / 2
        h = BOLT_HEIGHT / 2
        return bolt.isPlayerBolt() and (self.contains((bolt.x - w, bolt.y - h))
            or self.contains((bolt.x + w, bolt.y - h)) or self.contains((bolt.x
            + w, bolt.y + h)) or self.contains((bolt.x - w, bolt.y + h)))
...
class Bolt(GRectangle):
    ...
    setters for the velocities. That is because the velocity is fixed and cannot
    change once the bolt is fired.
    ...
    def getVelocity(self):
        """
        Returns velocity attribute of laser bolt
        """
    ...
    def __init__(self, x, y, width, height, fillcolor, velocity):
        """
        Initializes a Bolt which a GRectangle and sets the velocity of the bolt
        ...
        Parameter fillcolor: inside fill color of the bolt
        Precondtion: Must be an RGB value
        Parameter velocity: the direction and speed of the bolt being fired
        Precondition: Velocity is either a positive or negative int or float
        """
        super().__init__(x = x, y = y, width = width, height = height, fillcolor
            = fillcolor)
    ...
    def isPlayerBolt(self):
        """
        Return Boolean to determine if the bolt is a player bolt based on if
        the velocity is positive or negative
        """
        if self._velocity > 0:
            return True
        else:
